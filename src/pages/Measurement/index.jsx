...
const DeviceManager = () => {
    ...
    // New state for holding measurement data, lifted up
        ...
        const fetchInitialData = async () => {
            if (currentUser) {
                try {
                    ...
                    addLog(`Erro ao carregar medições iniciais: ${error.message}`, 'error');
                }
            }
        };
        fetchInitialData();
    ...
    const handleDownloadMeasurements = async () => {
        ...
        try {
            ...
            for (let i = 1; i <= measCount; i++) {
                ...
                // Save raw binary file locally
                const blob = new Blob([measurementData], { type: 'application/octet-stream' });
                const url = URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = `medicao_${i}_raw_${new Date().toISOString().replace(/[:.]/g, '-')}.bin`;
                a.click();
                URL.revokeObjectURL(url);

                addLog(`Arquivo binário da Medição ${i} salvo.`, 'success');
            }

            // After all measurements are downloaded, combine, parse, save, and display
            ...
            // Save parsed data
            if (currentUser && parsedArray.length > 0) {
                try {
                    await MeasurementService.saveMeasurements(currentUser.uid, parsedArray);
                    addLog(`${parsedArray.length} medições salvas no banco de dados.`, 'success');
                    // Update the state to trigger re-render in the list
                    // NOTE: A more robust solution might refetch, but this is good for now.
                    const freshData = await MeasurementService.getUserMeasurements(currentUser.uid);
                    setMeasurementData(freshData);

                } catch (error) {
                    addLog(`Erro ao salvar medições: ${error.message}`, 'error');
                }
            }

            // Format for display using the utility function
            const formattedDisplay = parsedArray.map(m =>
                `G${m.group} P${m.point}: ${formatResistance(m.resistance)}, ${m.current.toFixed(4)}A [${m.timestamp.toLocaleString()}]`
            ).join('\n');
    ...
    return (
        <div style={{ padding: 'var(--spacing-xl)' }}>
                        ...
                        {displayedMeasurementData && (
                            <div style={{ marginTop: '1rem' }}>
                                <label style={{ display: 'block', marginBottom: '0.5rem', color: 'var(--color-text-secondary)', fontSize: '0.875rem' }}>
                                    Dados Processados (Grupo, Ponto, Resistência, Corrente, Timestamp)
                                </label>
                                <textarea
                                    readOnly
            ...
            <MeasurementList data={measurementData} />
        </div>
    );
};
